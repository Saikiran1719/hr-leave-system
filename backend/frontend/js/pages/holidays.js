// frontend/js/pages/holidays.js — renderHolidays defined in balances.js
    ...
    try {
      ...
      <div class="page-anim" style="max-width:760px">
        ...
        <div class="card" style="margin-bottom:14px">
          <div style="display:flex;align-items:${isMobile()?'flex-start':'center'};gap:18px;margin-bottom:22px;flex-wrap:wrap">
            ${avatarHtml(getInitials(profile.FullName), 68)}
            <div style="flex:1">
              <div style="font-size:1.3rem;font-weight:800">${profile.FullName}</div>
          ...
          ` : `
            <div class="grid-3">
              ${[['Email',profile.Email],['Phone',profile.Phone||'—'],['Department',profile.DepartmentName||'—'],
                 ['Role',profile.Role?.toUpperCase()],['Manager',profile.ManagerName||'—'],
                 ['Joined',fmtDate(profile.JoinedDate)]].map(([k,v])=>`
                <div style="background:var(--bg);border-radius:var(--radius-sm);padding:12px">
                  ...
                  <div style="color:var(--text);font-weight:500;font-size:.88rem;word-break:break-all">${v}</div>
                </div>`).join('')}
            </div>`}
        </div>
        ...
        <div class="grid-4" style="gap:12px;margin-bottom:14px">
          ${[['Applied',myLeaves.length,'var(--primary)'],['Approved',ok.length,'var(--success)'],
             ['Pending',pen.length,'var(--warning)'],['Days Used',used,'var(--danger)']].map(([l,v,c])=>`
        ...
        <!-- Change Password -->
        <div class="card">
          <button id="pw-toggle-btn" style="width:100%;background:none;border:none;color:var(--text);cursor:pointer;
                  text-align:left;font-size:.95rem;font-weight:600;display:flex;justify-content:space-between;align-items:center">
          ...
          ${pwOpen ? `
            <div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border)">
              ...
              <button class="btn btn-primary" id="pw-save" style="width:auto;padding:10px 24px;margin-top:4px">Update Password</button>
            </div>` : ''}
        </div>
      </div>`;
      ...
      document.getElementById('pw-save')?.addEventListener('click', async () => {
        ...
        if (nw.length < 6) return toast.error('Password min 6 characters');
        try {
